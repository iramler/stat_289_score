...
team_data <- team_data %>% filter(season_type == 2)
```
...
```{r}
reg_season <- team_data %>% group_by(team_id, season) %>% tally()
```

```{r}
table(reg_season$n, reg_season$season)
```

```{r}
reg_season %>% filter(season == '2018', n == 33)
```
