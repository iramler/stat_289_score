...
```{r}
handball_df <- read_csv("~/Desktop/git/score_new/awsmit22/handball_module/data_repo_submission/handball_stats.csv")
```
...
```{r}
ggplot(data = handball_clean, aes(x = total_offense, y = HPI)) +
  geom_point() +
  geom_smooth(method = "lm") + theme_minimal() + labs(title = "Scatterplot of HPI against total_offense with regression line")

ggplot(data = handball_clean, aes(x = total_penalties, y = HPI)) +
  geom_point() +
  geom_smooth(method = "lm") + theme_minimal() + labs(title = "Scatterplot of HPI against total_penalties with regression line")
