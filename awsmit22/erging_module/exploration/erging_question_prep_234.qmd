...
1.  Make a side-by-side boxplot of the `Speed` for `Verified` and
...
```
  ...
  a. Describe the distribution of the plot.
...
2.  Find the summary statistics for `Speed` and calculate its IQR.
...
3.  Make a table of the top 10 speeds for rowers. Including only their `Name`, `Speed` and if their piece was `Verified`.
...
```
  ...
  a.  Using the table, identify the 4 outliers seen in the boxplot.
...
4.  Perform a t.test to evaluate if there is a significant difference in the
...
5.  Make a new dataset which does not have the 4 outliers in it.
...
```
  ...
  a.  With the new dataset make another side-by-side boxplot of the `Speed` for `Verified` and
...
```
  ...
  b.  Comment on the distribution of this new boxplot.

**The distribution is much more spread out without the outliers.**

6.  Perform a t.test with the new dataset and re-evalute if there is a
...
7.  What does the number of outliers for `Non-Verified` pieces say about the reliability of `Non-Verified` pieces in the dataset?
...
8.  Is it ethical to remove the outliers from the dataset as done in task 5?
