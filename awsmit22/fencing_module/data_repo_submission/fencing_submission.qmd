...
1.    Which of these histograms of `Victories` for each `Weapon` and `Gender` combination looks normally       distributed?

2.    The mean number of `Victories` for womens' foil is 11.5 and the standard deviation is 3.97. Can it       be concluded that the distribution of `Victories` for womens' foil is normal?

3.    The 5 number summary of `Victories` for womens' foil is provided below. Calculate the IQR of             `Victories` for womens' foil.

4.    Draw a boxplot of `Victories` for womens' foil. 

5.    Describe the trend of this scatterplot of `Victories` and `Ind`.

6.    The least squares line for `Victories` predicted by `Ind` is Victories = 11.5 + 0.16Ind. Interpret the slope of the line. 
