...
Make a df combining both the points and medals
```{r}
final_df
final

rowing_medals <- full_join(final, final_df)
```

```{r}
medals|>group_by(Event) |> summarise()
```

```{r}
ggplot(medals, aes(x = total_points)) + geom_histogram()
```
...
# Visualizations for the worksheet
