...
```{r}
rec_large <- glm(REC ~ SPEED + GENDER + DISTANCE, data = racewalking_df,
                     family = "binomial")
rec_large |> tidy()
```
...
```{r}
rec_large_int <- glm(REC ~ SPEED + GENDER + DISTANCE + GENDER:DISTANCE, data = racewalking_df,
                     family = "binomial")
rec_large_int |> tidy()
```
...
```{r}
rug_rec <- racewalking_df |> filter(REC == 1)
rug_norec <- racewalking_df |> filter(REC == 0)
ggplot(data = racewalking_df, aes(x = SPEED, y = REC)) +
...
```{r}
lgl <- racewalking_df |> mutate(REC = if_else(RECORD != "NONE", true = TRUE, false = FALSE))
lgl |> group_by(COUNTRY) |> summarise(total_rec = sum(REC))
```
