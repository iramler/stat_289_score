...
```{r}
record <- distance |> mutate(REC = if_else(RECORD != "NONE", true = 1, false = 0)) |> mutate(REC = as.factor(REC))
```

Create another binary (0,1) variable for if a racer had a podium finish, make sure to make the variable is a factor as well. 

```{r}
podium <- record |> mutate(PODIUM = if_else(POS <= 3, true = 1, false = 0)) |> mutate(PODIUM = as.factor(PODIUM))
```
