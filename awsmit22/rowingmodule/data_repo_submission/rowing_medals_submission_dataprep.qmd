...
```{r}
events_athletes <- read_csv("/Users/abigailsmith/Desktop/stat289/awsmit22/rowing_module/events_athletes.csv")
```
...
ggplot(data = medals, aes(x = total_medals)) + geom_histogram(bins = 20, colour = "black", fill = "lightblue") + theme_minimal() + labs(title = "Histogram of total_medals for all countries in all events")
```
...
```{r}
medals |> filter(NOC == "USA") 
```

```{r}
ggplot(medals, aes(x = NOC, y = total_medals)) + geom_col() + coord_flip()
```

```{r}
medals_reordered <- medals |> filter(total_medals != 0) |> mutate(NOC = fct_reorder(NOC, total_medals))
```

```{r}
ggplot(medals_reordered, aes(x = NOC, y = total_medals)) + geom_col() + coord_flip()
```
