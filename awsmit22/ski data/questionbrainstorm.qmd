...
# use of bib to predict rank4

```{r}
bibmod<- lm(Rank4~Bib, data = ski_df)
summary(bibmod)
```
...
```{r}
ski_long <- ski_df |> pivot_longer(c("Time1", "Time2", "Time3", "Time4"), names_to = "timeinterval", values_to = "time") |> relocate(time, timeinterval)
```

```{r}
skimod <- aov(Rank4 ~ time, data = ski_long)
summary(skimod)
```
