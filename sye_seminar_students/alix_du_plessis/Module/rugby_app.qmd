...
server <- function(input, output, session) {
  ...
  observeEvent(input$analyze, {
    ...
    opponent_team <- input$away_team

    win_count <- sum((match_data$home_team == selected_team & match_data$result == "Home Win") |
                     (match_data$away_team == selected_team & match_data$result == "Away Win"))
    loss_count <- sum((match_data$home_team == selected_team & match_data$result == "Away Win") |
                      (match_data$away_team == selected_team & match_data$result == "Home Win"))
    draw_count <- sum((match_data$home_team == selected_team | match_data$away_team == selected_team) & match_data$result == "Draw")
    ...
    font_theme <- theme_minimal(base_size = 16)
    ...
    output$win_prob <- renderText({
      paste0("Win Probability for ", selected_team, ": ", round(win_prob, 2), "%\n",
    ...
    output$extra_stats <- renderText({
      paste0("Average Score - ", input$home_team, ": ", round(avg_home_score, 1), "\n",
             ...
             "Total Matches Played: ", total_count)
    })
    ...
    output$win_margin_distribution <- renderPlot({
      match_data %>%
        ...
        labs(title = paste("Point Differential Distribution:", input$home_team, "vs", input$away_team),
             x = paste(input$home_team, "-", input$away_team), y = "Count", fill = "Score") +
        font_theme
    })
    ...
    output$result_pie_chart <- renderPlot({
      ...
      ggplot(pie_data, aes(x = "", y = n, fill = SelectedOutcome)) +
        ...
        labs(title = paste("Result Breakdown:", input$home_team, "vs", input$away_team),
             fill = paste("Outcome (from", input$home_team, "perspective)")) +
