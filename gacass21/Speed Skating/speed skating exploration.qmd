...
```{r}
time_outliers1 <- speed_skating |> filter(!is.na(time_lap1)) |> filter(time_lap1 < 9)
...
ggplot(data = time_outliers2, aes(x = time_lap2)) +
  geom_histogram(bins = 75)
```
...
speed_skating |> filter(!is.na(Time)) |> filter(Time == min(Time)) |>
  select(time_lap1, time_lap2, time_lap3, time_lap4, time_lap5)
```
...
```{r}
speed_skating |> filter(!is.na(time_lap1)) |> summary(time_lap1)
```
