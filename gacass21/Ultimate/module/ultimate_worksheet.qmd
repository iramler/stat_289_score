...
```{r}
ultimate_statistics <- read_csv(here('gacass21/Ultimate/module/ultimate_total.csv'))
ultimate_games <- read_csv(here('gacass21/Ultimate/module/ultimate_per_game.csv'))
```


## Tidy and Clean Data

Starting with the ultimate_statistics table, make the table in a tidy format using the pivot_wider() function, use pivot wider on ultimate_games as well.

Use a join to combine the 2 tables by Player, make sure to remove any columns that are repeated or that have the same information.
...
1. The data set has 1,665 total players that participated in the College Ultimate Championships, figure out how many actually contributed in a statistical category using a filter function. (Don't include team games as a category)
...
a. Make a plot that compares turns_per_game and Assists, label the players in the top 10 for assists using geom_text().
...
b. With the information that plus minus is only counted for offensive possessions where teams are more likely to score, what do the total plus minus numbers say about the dominance of offense in each division?
