...
timeout1 <- timeout1 %>% 
  select(prev_home_score, prev_visiting_score, losing_score, losing_team, set_winner, losing_team_wins)
```
...
statistic_function <- function(data, indices) {
  ...
  mean_score <- mean(sample_data$losing_score)
  return(mean_score) # look at the mean prev score
}
