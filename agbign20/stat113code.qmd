...
cdsports <- newAllSportFilms %>% mutate(ComedyIND = ifelse(str_detect(genres, "Comedy"), "Comedy", NA),
                                     DramaIND = ifelse(str_detect(genres, "Drama"), "Drama", NA)) %>%
  pivot_longer(cols=ComedyIND:DramaIND,names_to = NULL, values_to = "Type") %>%
  drop_na("Type") %>% select(primaryTitle,Year, Type, averageRating, sport) %>%
  filter(sport %in% sports)
...
With type held constant, average rating is significantly different for at least one sport.
The interaction between type and sport is not significant.
