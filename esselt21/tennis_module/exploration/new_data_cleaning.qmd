...
wta <- read_csv(here::here("esselt21/tennis_module/exploration/wta_2023.csv"))
```
...
atpClean <-
  atpClean |>
  relocate(loser_hand, .after = winner_hand)
```
...
atpClean <-
  atpClean |>
  mutate(breakPoints_saved = if_else(result == "Winner", w_bpSaved, l_bpSaved)) |>
  relocate(breakPoints_saved, .after = serve_games_won) |>
...
atpClean <-
  atpClean |>
  mutate(breakPoints_faced = if_else(result == "Winner", w_bpFaced, l_bpFaced)) |>
  relocate(first_serve_in, .after = breakPoints_saved) |>
...
atp <-
  atp |>
  relocate(break_points_saved, .after = first_serve_in)
=======
atp <-
  atp |>
  relocate(result, .after = surface) |>
  relocate(player, .after = result)
>>>>>>> ca93f92d5574e031a44b8a4c811a25f7ae0137b5
```

```{r}
write_csv(atp, here::here("esselt21/tennis_module/exploration/atp_2023.csv"))
```
