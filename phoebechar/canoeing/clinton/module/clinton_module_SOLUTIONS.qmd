...
#### 4. From 2014-2019, the Clinton tracked teams who registered but did not start or did not finish the race by marking them as DNS or DNF, which is entered under 'status' in the data. Modify the data set to only include teams with an NA under 'status'.
...
#### 6. We can see a very short bar at time == 0 (which is obviously impossible), indicating one boat with an unrecorded time slipped through our earlier filtering. Filter this boat out and remake the histogram.
...
##### 6.1. Change the number of bins (test out a few options!)
