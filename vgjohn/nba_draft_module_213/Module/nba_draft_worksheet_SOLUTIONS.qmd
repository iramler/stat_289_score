...
For this activity, you will be using data from the data set `nba_draft.csv`. The data set includes data from players that were selected in the NBA drafts between the years 1990-2021. Each row represents a different player and statistics from their respective careers in the NBA, some of which are still ongoing. Some key variables include `mins_per_game` the avergae number of minutes played per game in a player's career, `round_picked`, which shows what round the player was selected in, and `pick_in_round` which shows what number pick a player was in the round in which they were selected. 
...
# nba_draft <- read_csv( "/the path to the file")
```
...
* From looking at the means in the density plot and box plot, we are most likely to find a difference between those selected 1-10 and those selected 20-30 because there is the largest difference in mean number of minutes played. We are least likely to find a difference between those selected 10-20 and 20-30 because the difference in mean is the smallest. 

**8. In the 2011 draft, Kyrie Irving was selected 1st in the first round by the Cleveland Cavaliers, and Klay Thompson was selected 11th by the Golden State Warriors. Is there a significant difference between the number of minutes we would expect them to play? If so, what is the difference? Provide evidence as well as a specific number.**

```{r}
TukeyHSD(mpg_aov)
```

* From the output, we can see that there is a significant difference between the number of minutes played by Kyrie Irving (who was chosen 1-10), and Klay Thompson (who was chosen 10-20). We know this because the p-value of that specific comparison is significant (0.0000). From the output, the difference between number of minutes played between those who were selected 1-10 and 10-20 is 7.035, with those who were selected 1-10 playing on average 7.035 more minutes. So we would expect Kyrie Irving to be that much more. 

**9. Were your predictions in question 7 correct? Are there any comparisons that don't have a significant difference?** 

* The predictions from questions seven were correct. The difference in minutes played between those picked 1-10 and 20-30 was the largest (8.98), and the difference in minutes played between those picked 10-20 and 20-30 is the smallest (1.95). All of the differences are significant, with p-values below 0.05. 
