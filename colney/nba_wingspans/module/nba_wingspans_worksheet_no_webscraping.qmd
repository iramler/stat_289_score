---
title: "Wingspan & Performance in the NBA (no webscraping)"
format: html
---
...
# 1. Load Player Profiles csv file.
...
a.  Load the data from the `______.csv` file from the data folder.
...
c.  Do the same steps as in parts **a** and **b**, but this time use the `______.csv` file from the data folder.

**Note:** This tables contain **two** rows of column headers. To fix this issue, use the `row_to_names()` function. As a result some columns will have the same names, the `clean_names()` function fixes this issue.
...
c. Create a scatterplot using `wingspan_inches` as the explanatory variable and `3pt_percentage` as the response variable. Include a regression line, a title, and labels for the x and y axes. Then use `facet_wrap()` to sepeate the plot by `position`. What patterns or relationships stand out within or across positions?
```{r}
