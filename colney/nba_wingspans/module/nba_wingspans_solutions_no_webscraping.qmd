...
**Hint:** Use `group_by(player)` to group rows by player name. Then use `slice_max()` and order by games to keep only the row wheer each player had the most games played, which will be their season total. Finally, use `ungroup()` to remove the grouping.
