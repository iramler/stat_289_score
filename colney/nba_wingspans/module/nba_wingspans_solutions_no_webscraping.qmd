...
- First use `anti_join()` to identify any rows in the wingspan dataset with invalid team names. Then remove those rows from the dataset with `semi_join()`.
```{r}
teams <- read_csv(here::here("colney/nba_wingspans/data_repo_submission/nba_team_abbreviations.csv"))
...
```{r}
# We might need to provide a little more guidance here. I dont know if 
# most students would think of using slice_max without a hint
...
```{r}
# I think full join is actually the more natural one here. (not that it actually matters for the end 
# result in this case)
bball_ref <-
  per_100_poss %>%
...
In the 'nba_wingspan_2025.csv' file, two names were misspelled compared to their spellings on Basketball Reference. Keep this in mind as you work through the questions.

a.  Use `anti_join()` to combine the datasets. What do the resulting rows represent?
```{r}
# I think anti-join makes more sense here. this way you can identify the players without knowing them ahead of time
# Plus it looks like it shows that Harry Giles III is mistakenly split into a team called "III"

wingspan %>%
  anti_join(bball_ref, by = c("name" = "player"))
```

b.  **Optional:** Use `stringdist_join()` from the `fuzzyjoin` package to combine the datasets. What issues could this potentially cause?
...
```{r}
# This should probably be labeled as an "optional" problem.
# also need to load the package
