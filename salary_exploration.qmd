...
Data Link: https://www.kaggle.com/datasets/loganlauton/nba-players-and-team-data?select=NBA+Salaries%281990-2023%29.csv
...
final <-
  final |>
  mutate(sqrtInflationAdjSalary = sqrt(inflationAdjSalary / 10000))
```
...
```{r}
final |>
  ggplot(aes(x = sqrtInflationAdjSalary)) +
  geom_histogram(fill = "lightblue",
                 color = "black")
```
...
final <-
  final |>
  mutate(PER_Salary_ratio = PER / sqrtInflationAdjSalary)
```
...
final <-
  final |>
  mutate(logInflationAdjustedSalary = log(inflationAdjSalary / 1000) * 2)
```

```{r}
final <-
  final |>
  mutate(ESPR = PER / logInflationAdjustedSalary)
```
