...
Our goal is to try to predict the placement of the horses based on the betting odds and track conditions in a logistic regression model. 

To begin, let's use what we know about simple linear regression and predict whether a horse placed in the top 3 based on betting odds.

```{r}
# first load in any necessary packages and data
library(tidyverse)
# library(here)
# horseracing <- read.csv(here("bkaradenes/Horseracing_Module/module/triple-crown.csv"))
```
