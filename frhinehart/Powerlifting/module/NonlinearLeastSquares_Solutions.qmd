...
regression. XXXXXXX Add a brief description or reference to NLS in general.
XXXXXXX Non-linear regression modeling typically starts by identifying a
reasonable non-linear form for the model. In this case, we want a model
that allows for growth (increasing response value) followed by decay
(decrease in response value). One such function has the following form
...
to specify "initial values" to start the optimization.
